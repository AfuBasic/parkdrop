...
## Data Backup, Recovery & Operational Resilience Security (Build 23)
- **Quarantine Privacy & Isolation:** Quarantined rows (`quarantineRecords` store) contain malformed or corrupt payloads isolated from active application queries. Quarantined records are strictly scoped by `business_id` and never rendered in public operational screens.
- **Diagnostic PII Redaction:** Support diagnostic reports (`buildSanitizedReport()`) strictly redact customer phone numbers, package pickup codes, customer names, SMS tokens, and authentication cookies. Only sanitized error categories, redacted sample summaries, and masked entity identifiers (`PD-***`) appear in clipboard tokens (`PD-RCV-xxxx`).
- **Destructive Action Protection:** Database resets (`SafeResetDialog`) require explicit multi-step confirmation ("RESET DATA") and display honest counts of unsynced work across all businesses. Local mutations are never purged without explicit attendant consent.
- **Multi-Tab Concurrency Locking:** The Web Locks API (`navigator.locks.request`) with lease fallbacks prevents concurrent recovery scripts or rebootstrap routines from racing across browser tabs, preventing data duplication and split-brain states.
- **Honest Recovery Limits:** ParkDrop explicitly documents that offline unsynced work on a lost or destroyed physical device cannot be restored on a new device without prior synchronization to the server, and browser storage under severe OS disk pressure remains subject to eviction unless persistent storage permission is granted.
