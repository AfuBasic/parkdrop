...
class ReturnPackageAction
{
    /**
     * Authoritatively transition a WAITING package to RETURNED within a DB transaction and row-lock.
     *
    ...
    public function execute(
        ...
        return DB::transaction(function () use ($business, $package, $payload, $userId, $deviceUuid) {
            ...
            if ($lockedPackage->status !== 'WAITING') {
                throw new DomainException('PACKAGE_NOT_RETURNABLE');
            }

            // Validate reason
            $reasonVal = $payload['reason'] ?? null;
            $reason = $reasonVal instanceof PackageReturnReason
                ? $reasonVal
                : PackageReturnReason::tryFrom((string) $reasonVal);

            if (! $reason) {
                throw new InvalidArgumentException('INVALID_RETURN_REASON');
            }

            $reasonNote = isset($payload['reason_note']) ? trim($payload['reason_note']) : null;
            if ($reason === PackageReturnReason::OTHER && empty($reasonNote)) {
