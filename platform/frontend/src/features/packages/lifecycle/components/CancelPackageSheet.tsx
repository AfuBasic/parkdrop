...
export function CancelPackageSheet({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onConfirmCancel(selectedReason, selectedReason === 'OTHER' ? note.trim() : note.trim() || null);
      onClose();
    } catch (err) {
      setSubmitError(err instanceof Error ? err.message : 'Could not cancel package. Try again.');
    } finally {
      ...
      >
        ...
        <div className="flex items-center justify-between pb-2 border-b border-border-subtle">
          <div className="flex items-center gap-2 text-status-danger-text">
            ...
            <h2 id="cancel-sheet-title" className="text-lg font-bold text-text-primary">
              Cancel package
            </h2>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <LifecycleReasonField<CancelReason>
            legend="Why are you cancelling this package?"
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            <button
              ...
              className="flex-[1.4] py-3.5 px-3 rounded-[var(--radius-xl)] bg-status-danger-text hover:bg-status-danger-text/90 active:scale-[0.99] text-white font-bold transition-all shadow-sm cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed text-[15px] flex items-center justify-center gap-2 whitespace-nowrap shrink-0"
            >
              <Ban className="w-4 h-4 shrink-0" />
              <span className="whitespace-nowrap">{isSubmitting ? 'Cancelling…' : 'Confirm cancellation'}</span>
            </button>
          </div>
        </form>
      </div>
