...
export function usePickupIdentity(): PickupIdentity {
  ...
  const point =
    business?.pickup_points?.find((p) => p.status === "active") ??
    business?.pickup_points?.[0];

  const pointName =
    realNameOrNull(point?.name) ?? realNameOrNull(business?.name);
  const parkName = realNameOrNull(point?.park_name);
  const contactPhone = (point as any)?.contact_phone
    ? String((point as any).contact_phone).trim()
    : null;
